...
#define CW2225_ROM_FILE				"../cw_fls_algo/cw2225_rom.elf"
#define CW2245_ROM_FILE				"../cw_fls_algo/cw2245_rom.elf"
#define CW3065_ROM_FILE				"../cw_fls_algo/cw3065_rom.elf"
...
static int cw32_load_rom(struct flash_bank *bank)
{
	...
	if (strcmp(bank->driver->name, "cw2225") == 0) {
		rom_path = CW2225_ROM_FILE;
	}else if (strcmp(bank->driver->name, "cw2245") == 0) {
		rom_path = CW2245_ROM_FILE;
	}else if (strcmp(bank->driver->name, "cw3065") == 0) {
		rom_path = CW3065_ROM_FILE; 
	}else {
		return ERROR_FAIL;
	}

	log_info("load rom code %s", rom_path);
	LOG_DEBUG("load rom code %s", rom_path);
	full_path = find_file(rom_path);
	if (full_path == NULL) 
	{
	...
	for (unsigned int i = 0; i < image.num_sections; i++) {
		...
		if ((image.sections[i].base_address + buf_cnt >= min_address) &&
			...
			if (i==0) {
				// log_info("-----read first section in elf");
				// log_info("address " TARGET_ADDR_FMT " size %u bytes", (image.sections[i].base_address + offset)&0xFFFFFFFF, length);

				uint8_t bbb[0x5000] = {0};
				target_read_buffer(target,(image.sections[i].base_address + offset)&0xFFFFFFFF, length, bbb);

				// log_hex("buffer + offset", buffer + offset, 0x100);
				// log_hex("bbb            ", buffer + offset, 0x100);

				// log_info("-----read first section in fpga");

				uint32_t cmp_res = memcmp(buffer + offset, bbb, length);
				// log_info("cmp_res %u", cmp_res);

				if (cmp_res==0) {
					log_info("ROM check succeed, skip load!");
					break;
				}
			}

			retval = target_write_buffer(target,
					(image.sections[i].base_address + offset)&0xFFFFFFFF, length, buffer + offset);
			if (retval != ERROR_OK) {
				free(buffer);
				break;
			}
